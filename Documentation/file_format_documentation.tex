...
\item The value of every parameter is a list of values. These values are given as in the .dat format.
\end{enumerate}
...
\begin{enumerate}[i]
\item The file is a dictionary containing 6 keys, {\tt creation\_date}, {\tt additional\_timeseries}, {\tt index\_set\_dependencies}, {\tt start\_date}, {\tt timesteps} and {\tt inputs}.
...
\item The value of every input in {\tt inputs} is a list of dictionaries. Each of these dictionaries has the two keys {\tt indices} and {\tt values}. The value of {\tt indices} is a list of names of indexes for this timeseries. The value of {\tt values} is a list of numeric values. Unlike in the dat format, {\tt NaN} is not a valid json identifier, so {\tt null} is used to signify a missing value instead.
\end{enumerate}
