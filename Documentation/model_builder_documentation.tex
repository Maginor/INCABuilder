...
\apipar{}{None.}
\apiret{A {\tt inca\_model*} pointer to a newly created model object.}
...
\apipar{inca\_model *Model}{Pointer to the model object.}
...
\apipar{inca\_model *Model}{Pointer to the model object.}
\apidesc{Finalizes the model object. You can no longer call model building procedures on it, and you should not try to make any further changes to it in general. This procedure generates the final equation batch structure of the mode according to the rules given in Section \ref{sec:advancedbatchstructure}.}
}

\apientry{GenerateDataSet}{Application building procedure}{
\apipar{inca\_model *Model}{Pointer to the model object.}
\apiret{A {\tt inca\_data\_set*} pointer to a newly created dataset object.}
...
\apipar{inca\_data\_set *DataSet}{Pointer to the dataset object.}
...
\apipar{inca\_data\_set *DataSet}{The dataset you wish to run the model with.}
\apidesc{Attempts to run the model (that the dataset was generated from) with the parameter and input values given in this dataset. The start date is read from the "Start date" parameter if it exists, and the number of timesteps is read from the "Timesteps" parameter if it exists. All result timeseries are stored in the dataset.}
}

There are a few more procedures that we will try to document soon.

%TODO: These should possibly be documented:
%CopyDataSet(inca_data_set *)
%SetIndexes(inca_data_set *, token_string, vector<token_string>)
...
%SetParameterValue(inca_data_set *DataSet, const char *Name, const char * const *Indexes, size_t IndexCount, x Value)
%GetParameterX(inca_data_set *DataSet, const char *Name, const std::vector<const char *>& Indexes)
...
%PrintResultStructure(inca_model *)
%PrintParameterStorageStructure(inca_data_set *)
%PrintInputStorageStructure(inca_data_set *)
...
The registration procedures can only be called on a {\tt inca\_model} object that has not been finalized using {\tt EndModelDefinition}.
...
\apipar{parameter\_group\_h ParameterGroup}{Handle to the parameter group that the parameter should be put in.}
\apipar{const char *Name}{The full name of the parameter.}
\apipar{const char *Default}{The default value of the parameter. Must be on the form "Y-m-d".}
...
\apipar{inca\_solver\_setup\_function *SetupFunction}{One of the integrator setup functions provided by inca\_solvers.h or boost\_solvers.h, or others.}
...
for(<every index in that is an input branch to CURRENT_INDEX(IndexSet)>)
{
	Def
}
\end{SaveVerbatim}
...
\multicolumn{2}{|z|}{Note that it calls {\tt CURRENT\_INDEX(IndexSet)}, and thus registers an index set dependency.}\\
}
