...
AddSimplyHydrologyModule(inca_model *Model)
{
	...
	auto InitialSnowDepth        = RegisterParameterDouble(Model, Snow, "Initial snow depth as water equivalent", Mm, 0.0, 0.0, 10000.0);
...
//	ResetEveryTimestep(Model, DailyMeanSoilWaterFlow);
	...
	SetSolver(Model, GroundwaterVolume, SimplySolverLand);
	
	auto GroundwaterFlow          = RegisterEquation(Model, "Groundwater flow", MmPerDay);
	SetSolver(Model, GroundwaterFlow, SimplySolverLand);
	
	auto GroundwaterFlowToReach	  = RegisterEquation(Model, "Groundwater flow to reach per land class", MmPerDay);
	SetSolver(Model, GroundwaterFlow, SimplySolverLand);
	...
	EQUATION(Model, SoilWaterFlowToReach,
		return (1.0-PARAMETER(BaseflowIndex)) * RESULT(SoilWaterFlow) * PARAMETER(LandUseProportions);
	)
	...
	EQUATION(Model, GroundwaterFlowToReach,
		return RESULT(GroundwaterFlow) * PARAMETER(LandUseProportions);
	)
...
/* 	EQUATION(Model, ReachFlowInput,
	...
	EQUATION(Model, ReachFlow,
		...
		double ReachFlowInput = upstreamflow + RESULT(InfiltrationExcess) + RESULT(TotalSoilwaterFlowToReach) + RESULT(TotalGroundwaterFlowToReach);
		return
