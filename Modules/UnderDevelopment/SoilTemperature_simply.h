...
AddSoilTemperatureModel(inca_model *Model)
{
	...
	// To do: Maybe just fix this in the model and therefore have it
	// as an assumption rather than a parma that can vary?
	...
	// Replaced with:
	auto SoilThermalConductivityOverHeatCapacity = RegisterParameterDouble(Model, SoilTempParamsLand, "Soil thermal conductivity (W/m/°C, range 0.4-0.8) divided by soil specific heat capacity (MJ/m3/°C, range 1.0-1.3; more like 4-15 if there is freeze-thaw)"
	...
	EQUATION(Model, COUPSoilTemperature,
		...
		return LAST_RESULT(COUPSoilTemperature)
			+ 86400.0
				* Da / Square( 2.0 * PARAMETER(SoilDepth))
