...
{	
	...
	mtl4_ode_system(const inca_solver_equation_function &EquationFunction) : EquationFunction(EquationFunction)
	{
	}
...
struct mtl4_ode_system_jacobi
{
	...
	mtl4_ode_system_jacobi(const inca_solver_jacobi_function &JacobiFunction) : JacobiFunction(JacobiFunction)
	{
	}
    ...
    void operator()( const vec_mtl4 &X , mat_mtl4 &J , const double & /* t */ )
    {
		...
		inca_matrix_insertion_function MatrixInserter = [&](size_t Row, size_t Col, double Value) { Ins[Row][Col] = Value; };
...
INCA_SOLVER_FUNCTION(Mtl4ImplicitEulerImpl_)
{
...
INCA_SOLVER_SETUP_FUNCTION(Mtl4ImplicitEuler)
{
